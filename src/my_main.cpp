...
cv::VideoCapture cap("/home/linux/ros2_ws/src/segmentation/test.mp4")
...
int main(int argc, char** argv)
{
    ...
    //const std::string engineFileName = "/home/linux/ros2_ws/src/segmentation/models/segmenter_high.engine";
    //(3)
    //const std::string onnxFile = "/home/linux/ros2_ws/src/segmentation/models/segmenter_B-Mask_model.onnx";
    //const std::string engineFileName = "/home/linux/ros2_ws/src/segmentation/models/segmenter_B-Mask.engine";
    ...
    cv::VideoCapture cap("/home/linux/ros2_ws/src/segmentation/test.mp4");
    if (!cap.isOpened()) {
        std::cerr << "Failed to open video file: test.mp4" << std::endl;
        return false;
    }
    cv::Mat frame;
    // Step 3-2: 영상 저장
    ...
    while (rclcpp::ok()) {
        ...
        cv::imshow("Input Frame", frame);
        cv::imshow("Segmentation Image", sgmt_img);
        if (cv::waitKey(25) == 27) {  // ESC 키가 눌리면 종료
            break;
        }
    }
