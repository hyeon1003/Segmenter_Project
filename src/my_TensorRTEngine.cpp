...
bool TensorRTEngine::runInference(cv::Mat& frame, cv::Mat& result, cudaStream_t stream){
 int32_t nIO = engine->getNbIOTensors();
    ...
    colors[12]=cv::Vec3b(255,0,255); //human - 핑크색
    colors[33]=colors[15]=colors[56]=colors[64]=cv::Vec3b(255,255,0); //desk, table.. - 하늘색
    colors[19]=colors[30]=colors[31]=colors[75]=colors[101]=cv::Vec3b(0,255,255); // chair.. - 노란색
    colors[5]=cv::Vec3b(128,128,128); //ceiling - 회색
    colors[0]=cv::Vec3b(255,0,0); //wall - 파란색
    colors[3]=cv::Vec3b(255,255,255); //floor - 하얀색
    colors[10]=cv::Vec3b(0,255,0); //cabinet - 초록색
    colors[14]=cv::Vec3b(128,128,0); //door - 청록색
    colors[41]=cv::Vec3b(0,0,255); //box - 빨간색
    colors[50]=cv::Vec3b(0,94,255); //refrigerator - 주황색
    colors[74]=colors[143]=cv::Vec3b(255,0,95); //computer - 보라색
    colors[139]=cv::Vec3b(0,56,153); //fan - 갈색
